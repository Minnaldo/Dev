...
void update(int index, int start, int end, int diff, int change_idx)
{
    update_lazy(change_idx, start, end);
    ...
    if (start == end)
        return;
    int mid = (start + end) / 2;
    update(index * 2, start, mid, diff, change_idx);       // 왼쪾 자식 노드
    update(index * 2 + 1, mid + 1, end, diff, change_idx); // 오른쪽 자식 노드
}
