...
// TODO                검색? 조회?
    ...
    @Override
    public ArrayList<AnnounceVO> announceList() {
        ...
        try {
            ...
            while (rs.next()) {
                list.add(new AnnounceVO(rs.getInt("number"), rs.getString("title"), rs.getInt("view"), rs.getString("writer"), rs.getString("registDate")));
            }
        } catch (SQLException e) {
            e.printStackTrace();
        } finally {
    ...
    @Override
    public AnnounceVO announce(int number) {
        AnnounceVO an = null;
        String sql = "select * from announce where number=?";
        try {
            conn = ConnectionProxy.getConnection();
            ps = conn.prepareStatement(sql);
            ps.setInt(1, number);
            ...
            while (rs.next()) {
                an = new AnnounceVO(rs.getInt("number"), rs.getString("title"), rs.getInt("view"), rs.getString("writer"), rs.getString("date"));
            }
        } catch (SQLException e) {
            e.printStackTrace();
        } finally {
