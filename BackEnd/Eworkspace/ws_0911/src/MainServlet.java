...
public class MainServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		if (chk.equals("login")) {
			...
			String pw = req.getParameter("input_pw");
			...
			out.print("<!DOCTYPE html>" + "<html>" + "<head>" + "<meta charset='UTF-8'>" + "<title>도서 상세 정보 페이지</title>"
					...
					+ "width : 100%;" + "margin:auto;" + "}" + "table, th, td{" + "border: 1px solid black;" + "}"
					+ "td {" + "width : 150px;" + "height : 35px;" + "}" + ".index{" + "text-align : center;"
					+ "background-color : rgb(213,213,213);" + "}" + "tr{" + "width : 400px;" + "height : 35px;" + "}"
					+ "th {" + "height : 35px;" + "background-color : rgb(213,213,213);" + "}" + "table{"
					+ "width:500px;" + "margin:auto;" + "}" + "</style>" + "</head>" + "<body>"
					...
					+ "<tr><td class='index'>도서국가</td><td>" + nation + "</td></tr>"
					+ "<tr><td class='index'>출판일</td><td>" + publisheDate + "</td></tr>"
					+ "<tr><td class='index'>출판사</td><td>" + publisher + "</td></tr>"
					+ "<tr><td class='index'>저 자</td><td>" + author + "</td></tr>"
					+ "<tr><td class='index'>도서가격</td><td>" + price + currency + "</td></tr>"
					+ "<tr><td class='index'>도서설명</td><td>" + desc + "</td>" + "</tr>" + "</table></div>"
					+ "<br><div style='text-align:center;'><a href='./book.html'>도서 등록</a></div>" + "</body>"
					+ "</html>");
		}
