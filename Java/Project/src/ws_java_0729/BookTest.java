...
public class BookTest {
    private static void printFunc() {
        ...
        System.out.println("7. 올해 발간된 잡지만 검색");
        System.out.println("8. 출판사로 검색하는 기능 ");
        System.out.println("9. 가격으로 검색 기능 (파라미터로 주어진 가격보다 낮은 도서 정보 검색)");
    ...
    public static void main(String[] args) {
        ...
        ArrayList<Book> tmp = new ArrayList<>();    // 기능의 결과를 임시로 저장할 리스트
        ...
        while (!exitFlag) {
            ...
            switch (sc.nextInt()) {
                ...
                case 1:
                    ...
                    switch (sc.nextInt()) {
                        case 1:
                            System.out.println("1. book 데이터 입력 (isbn, title, author, publisher, price(int), desc");
                            String[] str = sc.next().toUpperCase().replaceAll(" ", "").split(",");
                            bm.add(new Book(str[0], str[1], str[2], str[3], Integer.parseInt(str[4]), str[5]));
                            break;
                        case 2:
                            System.out.println(
                                    "2. Magazine 데이터 입력(isbn, title, author, publisher, price(int), desc, year, month");
                ...
                case 7:
                    ...
                    tmp = bm.searchMagzineInthisYear(Calendar.getInstance().get(Calendar.YEAR));  // 날짜를 가져온다
                    for (Book b : tmp)
                        System.out.println(b.toString());
                    break;
                case 8:
                ...
                case 9:
                    ...
                    break;
                case 10:
                    System.out.println("책 가격의 합계는 " + bm.priceSum() + "원 입니다.");
                    break;
                case 11:
                    System.out.println("책 가격의 평균은" + bm.priceAvg() + "원 입니다.");
                    break;
                default:
                    System.out.println("잘못된 값을 입력하셨습니다. 올바른 값을 입력해 주세요");
                    break;
            }
        }
    }
