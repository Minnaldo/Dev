...
public class ProductTest {
    public static void main(String[] args) {
        ...
        while (!flag) {
            ...
            case 1:
                try {
                    System.out.println("1 : hw_java_0723.TV,  2 : hw_java_0723.Refrigerator,  0 : 돌아가기");
                    int fnum = sc.nextInt();
                    if (fnum == 1) { // hw_java_0723.TV 추가
                        System.out.println("\"제품번호,제품명,가격정보,재고수량,디스플레이크기,디스플레이타입\"형태로 입력해주세요");
                        String[] dataStr = sc.next().replaceAll(" ", "").split(",");
                        pm.addProduct(new TV(Integer.parseInt(dataStr[0]), dataStr[1].toUpperCase(),
                                ...
                                Integer.parseInt(dataStr[4]), dataStr[5].toUpperCase()));
                        System.out.println("TV를 추가하였습니다");
                    } else if (fnum == 2) { // hw_java_0723.Refrigerator 추가
                        System.out.println("\"제품번호,제품명,가격정보,재고수량,용량\"형태로 입력해주세요");
                        String[] dataStr = sc.next().replaceAll(" ", "").split(","); // 공백 제거후 스플릿
                        pm.addProduct(new Refrigerator(Integer.parseInt(dataStr[0]), dataStr[1].toUpperCase(),
                                Integer.parseInt(dataStr[2]), Integer.parseInt(dataStr[3]),
            ...
            case 4:
                ...
                break;
            case 5:
                System.out.println("hw_java_0723.TV 정보만 검색합니다.");
                System.out.println(pm.searchTV());
                break;
            case 6:
                System.out.println("hw_java_0723.Refrigerator 정보만 검색합니다.");
