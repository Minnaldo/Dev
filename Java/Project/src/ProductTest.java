                ...
                case 1:
                    try {
                        ...
                        if (fnum == 1) {  // TV 추가
                            ...
                            pm.addProduct(new TV(Integer.parseInt(dataStr[0]), dataStr[1], Integer.parseInt(dataStr[2]), Integer.parseInt(dataStr[3]), Integer.parseInt(dataStr[4]), dataStr[5]));
                            System.out.println("TV를 추가하였습니다");
                        } else if (fnum == 2) {  // Refrigerator 추가
                            System.out.println("\"제품번호,제품명,가격정보,재고수량,용량\"형태로 입력해주세요");
                            String[] dataStr = sc.next().split(",");
                            pm.addProduct(new Refrigerator(Integer.parseInt(dataStr[0]), dataStr[1], Integer.parseInt(dataStr[2]), Integer.parseInt(dataStr[3]), Integer.parseInt(dataStr[4])));
